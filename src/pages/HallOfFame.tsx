            ...
            {records.map(r => (
              <button
                ...
                style={{
                  ...
                  background: selectedId === r.id ? '#FCB514' : 'var(--surf2)',
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        {selected.final_standings.map((s, i) => (
                          <div key={i} style={{
                            ...
                            background: i === 0 ? 'rgba(252,181,20,0.07)' : i < 3 ? 'var(--surf2)' : 'transparent',
                    ...
                    <div style={{ marginBottom: 24 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {([['🏆', champion], ['🥈', runnerUp], ['🥉', third]] as [string, ResultEntry | null][]).filter(([, r]) => r).map(([icon, r]) => (
                          <div key={(r as ResultEntry).id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 10, background: icon === '🏆' ? 'rgba(252,181,20,0.07)' : 'var(--surf2)', border: icon === '🏆' ? '1px solid rgba(252,181,20,0.2)' : '1px solid var(--bdr)' }}>
