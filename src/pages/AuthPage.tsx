...
export default function AuthPage() {
  ...
  const backToLogin = () => { setForgot(false); setResetSent(false) }
            ...
            {resetSent ? (
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--tx2)', marginBottom: 20 }}>
                  If an account exists for <strong style={{ color: 'var(--tx1)' }}>{form.email.trim()}</strong>, a password-reset link is on its way. Check your inbox (and spam) and tap the link to set a new password.
                </p>
                <button onClick={backToLogin} className="btn-gold pressable" style={{ width: '100%', justifyContent: 'center', minHeight: 48 }}>
                  Back to sign in
                </button>
              </div>
            ) : (
              <form onSubmit={handleForgot}>
                <p style={{ fontSize: 13.5, lineHeight: 1.6, color: 'var(--tx3)', margin: '0 0 16px', textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  />
                  <button type="submit" className="btn-gold pressable" disabled={loading} style={{ width: '100%', justifyContent: 'center', minHeight: 48, marginTop: 4 }}>
                    {loading ? (
                  ...
                  <button type="button" onClick={backToLogin} style={{ background: 'none', border: 'none', color: 'var(--tx3)', fontSize: 13, cursor: 'pointer', marginTop: 2 }}>
                    ← Back to sign in
                  </button>
                </div>
              </form>
        ...
        {mode === 'login' && (
          <div style={{ textAlign: 'center', marginTop: 14 }}>
            <button type="button" onClick={() => { setForgot(true); setResetSent(false) }}
              style={{ background: 'none', border: 'none', color: 'var(--gold)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              Forgot password?
            </button>
          </div>
        )}
